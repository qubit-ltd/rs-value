...
impl ValueWirePayloadV1 {
    ...
    fn try_new(value: ValueContainer) -> Result<Self, ValueWireEncodeError> {
        let non_finite = match &value {
            ValueContainer::Scalar(Value::Float32(value)) => !value.is_finite(),
            ValueContainer::Scalar(Value::Float64(value)) => !value.is_finite(),
            ValueContainer::Collection(MultiValues::Float32(values)) => {
                values.iter().any(|value| !value.is_finite())
            }
            ValueContainer::Collection(MultiValues::Float64(values)) => {
                values.iter().any(|value| !value.is_finite())
            }
            _ => false,
        };

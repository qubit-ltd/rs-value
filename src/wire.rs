...
/// Serializes string maps by dictionary order so every serializer observes
/// the same map entry sequence.
pub(crate) mod string_map {
    ...
    pub(crate) mod vec {
        use super::{CanonicalStringMap, HashMap};
        use serde::ser::SerializeSeq;
        use serde::{Deserialize, Deserializer, Serializer};

        /// Serializes a collection of string maps canonically.
        pub(crate) fn serialize<S>(
...
pub(crate) use string_map::vec as string_map_vec;
...
pub(crate) mod json {
    ...
    pub(crate) mod vec {
        use super::{CanonicalJson, JsonValue};
        use serde::ser::SerializeSeq;
        use serde::{Deserialize, Deserializer, Serializer};

        /// Serializes a collection of JSON values canonically.
        pub(crate) fn serialize<S>(values: &[JsonValue], serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            let mut sequence = serializer.serialize_seq(Some(values.len()))?;
        ...
        pub(crate) fn deserialize<'de, D>(deserializer: D) -> Result<Vec<JsonValue>, D::Error>
        where
            D: Deserializer<'de>,
        {
            Vec::<JsonValue>::deserialize(deserializer)
        }
    }

    /// Returns a recursively canonical JSON value for natural projections.
    pub(crate) fn canonicalize_json_value(value: &JsonValue) -> JsonValue {
        match value {
...
#[cfg(feature = "json")]
pub(crate) use json::vec as json_vec;

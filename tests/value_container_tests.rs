...
use qubit_value::{
    MultiValues, NamedMultiValues, NamedValue, Value, ValueContainer, ValueError,
...
fn test_value_container_tagged_wire_preserves_shape() {
    ...
    assert_eq!(
        serde_json::to_value(ValueWireV1::try_from(scalar).expect("scalar should fit V1"),)
            .expect("serialize scalar"),
        json!({"version": 1, "value": {"scalar": {"int32": 42}}})
    );
    assert_eq!(
        serde_json::to_value(ValueWireV1::try_from(collection).expect("collection should fit V1"),)

...
fn test_finite_float_adapters_round_trip_finite_values() {
    for value in [Value::Float32(1.25), Value::Float64(-2.5)] {
        let json = serde_json::to_value(&value).expect("finite scalar must serialize");
        let actual: Value = serde_json::from_value(json).expect("finite scalar must deserialize");
    ...
    for values in [
        ...
        let json = serde_json::to_value(&values).expect("finite collection must serialize");
        let actual: MultiValues =
            serde_json::from_value(json).expect("finite collection must deserialize");

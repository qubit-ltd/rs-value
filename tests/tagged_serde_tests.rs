...
fn value_fixtures() -> Vec<(Value, JsonValue)> {
    vec![
        (Value::Empty(DataType::Int32), json!({"Empty": "int32"})),
        ...
        (
            Value::StringMap(HashMap::from([("key".to_string(), "value".to_string())])),
...
fn multi_values_tagged_json_matches_all_variant_fixtures() {
    for (value, expected_value) in value_fixtures() {
        ...
        let expected = match expected_value {
            JsonValue::Object(object) if !object.contains_key("Empty") => {
                let (variant, item) = object.into_iter().next().unwrap();
                JsonValue::Object(Map::from_iter([(variant, JsonValue::Array(vec![item]))]))
            }
            expected => expected,
        };

        let encoded = serde_json::to_value(&values)
            .unwrap_or_else(|error| panic!("failed to serialize {values:?}: {error}"));
...
fn tagged_wide_integer_payloads_require_decimal_strings() {
    ...
    assert!(serde_json::from_value::<MultiValues>(json!({"UInt128": ["1", 2]})).is_err());
}

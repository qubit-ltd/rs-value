...
fn container_from_bytes(data: &[u8]) -> ValueContainer {
    ...
    let duration = Duration::new(seed % 1_000_000, (seed % 1_000_000_000) as u32);
    ...
    let url = Url::parse("https://example.com/fuzz").expect("fixed fuzz URL must be valid");
    let string_map = HashMap::from([("key".to_owned(), text.clone()), ("seed".to_owned(), seed.to_string())]);
    ...
    match tag {
        ...
        25 => ValueContainer::Scalar(Value::new_unset(Value::Int32(0).data_type())),
        26 => ValueContainer::Collection(MultiValues::Bool(vec![true, false])),
        27 => ValueContainer::Collection(MultiValues::Char(vec![character, '🦀'])),
        28 => ValueContainer::Collection(MultiValues::Int8(vec![number as i8, (number as i8).wrapping_add(1)])),
        29 => ValueContainer::Collection(MultiValues::Int16(vec![number as i16, (number as i16).wrapping_add(1)])),
        30 => ValueContainer::Collection(MultiValues::Int32(vec![number, number.wrapping_add(1)])),
        31 => ValueContainer::Collection(MultiValues::Int64(vec![number64, number64.wrapping_add(1)])),
        32 => ValueContainer::Collection(MultiValues::Int128(vec![number128, number128.wrapping_add(1)])),
        33 => ValueContainer::Collection(MultiValues::UInt8(vec![seed as u8, (seed as u8).wrapping_add(1)])),
        34 => ValueContainer::Collection(MultiValues::UInt16(vec![seed as u16, (seed as u16).wrapping_add(1)])),
        35 => ValueContainer::Collection(MultiValues::UInt32(vec![seed as u32, (seed as u32).wrapping_add(1)])),
        ...
        38 => ValueContainer::Collection(MultiValues::Float32(vec![float32, float32 + 0.5])),
        39 => ValueContainer::Collection(MultiValues::Float64(vec![float64, float64 + 0.5])),
        40 => ValueContainer::Collection(MultiValues::BigInteger(vec![big_integer])),
        41 => ValueContainer::Collection(MultiValues::BigDecimal(vec![big_decimal])),
        ...
        45 => ValueContainer::Collection(MultiValues::DateTime(vec![date_time])),
        ...
        49 => ValueContainer::Collection(MultiValues::StringMap(vec![string_map])),
        50 => ValueContainer::Collection(MultiValues::Json(vec![json])),
        _ => ValueContainer::Collection(MultiValues::new_unset(MultiValues::Int32(Vec::new()).data_type())),
    }
}
...
fuzz_target!(|data: &[u8]| {
    ...
    match wire {
        Ok(wire) => {
            ...
            let decoded = ValueWireV1::decode_json_slice_with_limits(&encoded, fuzz_decode_limits())
